...
1. Re-read the spec sections relevant to this phase (§15 defines phase scope) and the Hard Constraints (§3).
...
4. Run the quality gates and fix all findings:
   - `uv run ruff check .` and `uv run ruff format .`
   - `uv run mypy src/`
   - `uv run pytest` (offline, mock providers only)
5. Self-review against every HC the phase touches; list each one with a pass/fail verdict and evidence (file + line or test name).
6. Present a short completion report: what was built, deliberate deviations with rationale, and open questions.
